...
function travel(newLocation, connections, alt_click)
    ...
    if campaign == -1 then
        if newLocation ~= "Boulder Field" and newLocation ~= "Lone Tree Station" then
            broadcastToAll("For the Prologue you can only travel to Boulder Field and Lone Tree Station", Color.Red)
            return
...
function setupLocation(location, connection, skipLocationCard)
    ...
    campaignTracker.UI.setAttribute("location", "text", location)
    campaignTracker.UI.setAttribute("terrain", "text", connection)
        ...
        local function getMissionStartDay(missionName)
            for i = 1, 33 do
                ...
                if name == missionName then
                    return campaignTracker.UI.getAttribute("mission"..i.."Day", "text")
                end
            end
            return nil
        end
    ...
    -- Make sure prologue doesn't include location set cards
